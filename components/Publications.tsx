...
function authorProcess(authorsStr: string, personalInfoName: string): string {
  ...
  const normalizedPersonalName = personalInfoName.trim().replace(/\s+/g, ' ');

  const boldedAuthors = authors.map((author) => {
    const reorderedAuthor = author
      .trim()
      .split(',')
      .map((part) => part.trim())
      .reverse()
      .join(' ')
      .trim();
    const cleanAuthor = reorderedAuthor.replace(/{|}/g, '');
    const normalizedAuthor = cleanAuthor
      .replace(/\s*[*†]+$/u, '')
      .trim()
      .replace(/\s+/g, ' ');
    const marker = cleanAuthor.match(/\s*([*†]+)$/u)?.[1] ?? '';

    if (normalizedAuthor === normalizedPersonalName) {
